...
|---|---|---|
| 1.0 | 2025-07-13 | 初版 |
| 1.1 | 2026-04-22 | テーマシステム対応: `themes` / `user_preferences` テーブル追加、`system_settings.default_theme_id` キー追加 |
...
Users (利用者・ヘルパー・ケアマネージャー・システム管理者)
  ...
  ├─ 1:N ─ UserPreferences (設定・所有者として)
  ├─ 1:N ─ Themes (カスタムテーマ・作成者として)
  └─ 1:N ─ SystemSettings (設定・更新者として)

Themes (テーマ)
  └─ 参照 ─ UserPreferences.preference_value (preference_key='theme_id')
...
### 3.20 Themes（テーマ）
**ページデザイン（配色・書体・余白・角丸）を定義するテーマを管理**

詳細仕様は [`theme_system_specification.md`](./theme_system_specification.md) §3 を参照。
...
CREATE TABLE themes (
    ...
    theme_key VARCHAR(40) NOT NULL UNIQUE
        CHECK (theme_key ~ '^[a-z0-9_-]{2,40}$'),
    name VARCHAR(60) NOT NULL,
    description TEXT,

    -- テーマ定義（JSON スキーマは theme_system_specification.md §3.1 参照）
    definition JSONB NOT NULL,

    -- 区分
    is_builtin BOOLEAN NOT NULL DEFAULT false,  -- 組込みテーマ（削除不可）
    is_active BOOLEAN NOT NULL DEFAULT true,    -- ユーザー選択可能か
...
CREATE INDEX idx_themes_builtin ON themes(is_builtin);
```

**設計ポイント**:
- `theme_key` は URL 安全な識別子（プレビューAPI・外部連携で使用）
- `is_builtin=true` のレコードはマイグレーションシードで投入し、API からの削除不可（更新は name / description / is_active のみ許可）
- 初期投入プリセット: `standard`, `high-contrast`, `warm`, `calm`（`theme_system_specification.md` §4 参照）
- `definition` は JSONB（GIN インデックスは初期段階では不要。将来のタグ検索用に追加余地）

### 3.21 UserPreferences（ユーザー設定）
**ユーザーごとの個人設定を key-value で管理（テーマ選択・その他）**
...
**設計ポイント**:
- 汎用 key-value 形式。`theme_id` のほか将来の `locale` / `notification_channel` 等も同テーブルで扱う
- 既知のキー:
  - `theme_id`: ユーザー選択テーマの `themes.theme_key`（文字列）
  - `font_size_override`: `"normal" | "large" | "x-large"`（将来）
- `preference_value` は JSONB とし、値の型はキーごとにアプリ層でバリデーション
- `user_id` カスケード削除で退会時に自動クリーンアップ

### 3.22 SystemSettings 追加キー（テーマシステム関連）
`system_settings` テーブル（§3.15）に以下のキーを追加する（DDL 変更なし、データ追加のみ）。

| setting_key | category | デフォルト | 説明 |
|---|---|---|---|
| `default_theme_id` | general | `"standard"` | ログイン前および未設定ユーザーに適用する既定テーマの `theme_key` |
...
12. **Themes, UserPreferences** - テーマシステム（[テーマシステム仕様書](./theme_system_specification.md)参照）
